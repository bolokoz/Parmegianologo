    ...
    <div class="container">
      <div class="flex flex-col">
          ...
          <a :href="item.node.path" class="w-full md:flex">
            ...
            >
              <p class="text-black font-bold text-lg mb-2">
                {{ item.node.title }} - {{ item.node.cidade }}
              </p>
              <p class="text-gray-600 text-sm leading-none mb-2">
                {{ item.node.date }}
              </p>
        ...
        <div class="flex content-center justify-center">
          <Pager
            linkClass="px-2 py-1 m-1 rounded-full bg-green-300 text-green-700"
...
query Article ($page: Int) {
	posts: allArticlePost( page: $page) @paginate {
    pageInfo {
      totalPages
      currentPage
    }
